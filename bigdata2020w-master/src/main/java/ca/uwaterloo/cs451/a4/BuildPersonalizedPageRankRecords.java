...
public class BuildPersonalizedPageRankRecords extends Configured implements Tool {
  ...
  private static final String SOS = "sources";
  ...
  private static class MyMapper extends Mapper<LongWritable, Text, IntWritable, PageRankNode> {
    ...
    private static final ArrayList<Integer> srcs= new ArrayList<Integer>();
    @Override
    ...
    public void map(LongWritable key, Text t, Context context) throws IOException,
      ...
      float[] pr= new float[srcs.size()];
      for(int i=0;i<srcs.size();i++) {
      if(node.getNodeId()==srcs.get(i)) {
        pr[i]=0.0f;
      } else {
        pr[i]=Float.NEGATIVE_INFINITY;
      }
      }
      node.setPageRank(new ArrayListOfFloatsWritable(pr));
