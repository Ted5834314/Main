...
public class RunPersonalizedPageRankBasic extends Configured implements Tool {
  ...
  private static class MapClass extends
    ...
    public void setup(Mapper<IntWritable, PageRankNode, IntWritable, PageRankNode>.Context context) {
      ls=context.getConfiguration().getInt("LS", 0);
	...
	ArrayListOfFloatsWritable pr = node.getPageRank();
	float[] mass = new float[ls];
        for (int i=0;i<ls;i++) {
          mass[i]=pr.get(i)-(float) StrictMath.log(list.size());
        }
  ...
  private static class CombineClass extends
    ...
    public void reduce(IntWritable nid, Iterable<PageRankNode> values, Context context)
        throws IOException, InterruptedException {
      int massMessages = 0;
      int ls=context.getConfiguration().getInt("LS", 0);
  ...
  private static class ReduceClass extends
	...
	for(int i=0;i<ls;i++) {
	  totalMass.set(i,sumLogProbs(totalMass.get(i),mass[i]));
	}
  ...
  private static class MapPageRankMassDistributionClass extends
    ...
    public void setup(Context context) throws IOException {
      Configuration conf = context.getConfiguration();
      String[] smiss = conf.getStrings("MissingMass", "");
      String[] SOS = conf.getStrings("SRC", "");
      for(String ms:smiss) {
        missingMass.add(Float.valueOf(ms));
      }
      nodeCnt = conf.getInt("NodeCount", 0);
       for(String s:SOS) {
        src.add(Integer.valueOf(s));
      }
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
      ...
      p.set(i,sumLogProbs(jump, link));
       ...
       p.set(i,((float) Math.log(1.0f - ALPHA)) + p.get(i));
  ...
  private void iteratePageRank(int i, int j, String basePath, int numNodes,
    ...
    for (int t=0;t<mass.size();t++) {
      float ms=1.0f - (float) StrictMath.exp(mass.get(t));
      missing.add(ms);
    }
  ...
  private ArrayList<Float> phase1(int i, int j, String basePath, int numNodes,
    ...
    job.getConfiguration().setInt("LS", interm.length);
  ...
  private void phase2(int i, int j, ArrayList<Float> missing, String basePath, int numNodes, String src) throws Exception {
    Job job = Job.getInstance(getConf());
    ...
    for (int t=0;t<missing.size();t++) {
      smiss+=String.valueOf(missing.get(t));
      if(t<missing.size()-1) {
        smiss+=",";
      }
    }
    ...
    job.getConfiguration().setBoolean("mapred.map.tasks.speculative.execution", false);
    job.getConfiguration().setBoolean("mapred.reduce.tasks.speculative.execution", false);
    job.getConfiguration().setStrings("MissingMass", smiss);
    job.getConfiguration().setInt("NodeCount", numNodes);
    job.getConfiguration().setStrings("SRC", src);
